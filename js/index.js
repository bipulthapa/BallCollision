...
class Ball{
    ...
    init(){
        ...
        this.element.style.fontSize = (this.width-this.width/2)+"px";
        this.element.style.textAlign = "center";
        this.element.style.fontWeight = "bold"; 
        //for deleting the element antSmasher
    ...
    draw(){
        ...
        // this.element.style.backgroundImage = "url(./images/antImage.png)";
...
for(var i=0;i<ballCount;++i){
    var radius = getRandomArbitary(20,40);
        ...
        for(var k=0;k<ballsArray.length;k++){
            oldBall = ballsArray[k];
                if(tempBall.isTouching(oldBall)){
                 radius = getRandomArbitary(20,30);
        ...
        for(var i = 0; i < ballsArray.length; i++) {
            if(i != index) {
                if(value.isTouching(ballsArray[i])) {
                    ...
                    // if (value.velocity.x == ballsArray[i].velocity.x){
                    //     value.velocity.x *= getRandomArbitary(0, 1.5)
                    //     value.velocity.y *=Math.random();
                    // }
